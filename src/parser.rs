...
impl<'a, 'b> Parser<'a, 'b> {
    ...
    pub fn parse_file(&mut self) -> Option<File<'a>> {
        ...
        let mut resource_decls = vec![];
        let mut using_decls = vec![];
        ...
        loop {
            ...
            if self.last_token.subkind == TokenSubkind::Library {
            ...
            } else if self.last_token.subkind == TokenSubkind::Using {
                if let Some(decl) = self.parse_using_declaration(attributes.take()) {
                    using_decls.push(decl);
                } else {
                    self.last_token = self.lexer.lex();
                }
                ...
                || self.last_token.subkind == TokenSubkind::Overlay
            {
            ...
            } else if self.last_token.subkind == TokenSubkind::ResourceDefinition {
                if let Some(decl) = self.parse_resource_declaration(attributes.take()) {
    ...
    pub fn parse_union_member(
        ...
        let name_or_reserved = match self.parse_identifier() {
            ...
            None => {
                panic!(
                    "Failed to parse identifier for union member. Last token: {:?}",
                    self.last_token
                );
            }
        };
    ...
    pub fn parse_layout_parameter(&mut self) -> Option<LayoutParameter<'a>> {
        ...
        if self.last_token.subkind == TokenSubkind::Struct
            && self.peek_token().kind == TokenKind::LeftCurly
        {
        ...
        if attrs.is_some() || !mods.is_empty() {
        ...
        if self.last_token.kind == TokenKind::Identifier {
            ...
            Some(LayoutParameter::Identifier(id))
            ...
            || self.last_token.subkind == TokenSubkind::False
        {
            ...
            Some(LayoutParameter::Literal(LiteralConstant {
    ...
    pub fn parse_constant_term(&mut self) -> Option<Constant<'a>> {
        ...
        } else if self.last_token.kind == TokenKind::LeftParen {
            self.consume_token(TokenKind::LeftParen)?;
    ...
    pub fn parse_modifiers(&mut self) -> Vec<Modifier<'a>> {
        ...
        loop {
            let subkind = self.last_token.subkind;
            match subkind {
                TokenSubkind::Strict
                ...
                | TokenSubkind::Open => {
                    ...
                    self.consume_token_with_subkind(subkind);
